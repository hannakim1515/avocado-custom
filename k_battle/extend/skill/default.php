...
if($sk['si_category']==980){
    ...
    if($sk['si_1']=='default'){
        ...
        for ($i=0; $i < count($target); $i++) {
            ...
            $dead_msg=set_k_dmg($target[$i], 'hp', $result, $sk['si_code']);
        ...
        for ($i=0; $i < count($target); $i++) {
            if($bonus_loop){
                $bonus_stat=$target[$i][$k_unit_stat[$sk['target_sc']]];
                $bonus = round(($bonus_stat ? $bonus_stat : 1) * $sk_value)-$target[$i][$k_unit_stat[$sk['target_sc']]];
                if($bonus<0){
                    $func_type="minus";
                }else{
                    $func_type="plus";
