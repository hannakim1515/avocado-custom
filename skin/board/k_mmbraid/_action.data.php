...
// 레이드 액션 스킨 include
if (!empty($log_comment['wr_k_raid_log']) || !empty($log_comment['wr_k_raid_admin'])) {
    $raid_action = G5_PATH.'/k_battle/skin_raid/raid.mmbraid_action.php';
    include($raid_action);
}

if($log_type) { 
	// 허용된 로그 타입만 include (보안)
	$allowed_log_types = array('S', 'H', 'A', 'D', 'U');
	if (in_array($log_type, $allowed_log_types) && file_exists($board_skin_path."/action/log.{$log_type}.skin.php")) {
