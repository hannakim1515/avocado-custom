...
// 코멘트 출력 (SQL Injection 방지)
//$sql = " select * from {$write_table} where wr_parent = '{$wr_id}' and wr_is_comment = 1 order by wr_comment desc, wr_comment_reply ";
$wr_parent_safe = (int)$list_item['wr_id'];
$sql = " select * from {$write_table} where wr_parent = '{$wr_parent_safe}' and wr_content != '' order by wr_datetime asc ";
...
for ($c_i=0; $c_row=sql_fetch_array($result); $c_i++)
{
	...
	if ($board['bo_use_sideview'])
		$comment[$c_i]['name'] = get_sideview($c_row['mb_id'], $tmp_name);
	else
