...
// 일반 공격 처리 부분
$m_log_comment = "{$ds['dg_d_attack_comment']}&&&&";

// 공격력 버프 확인
$buff_attack = get_status_buffer_enermy($ds_id, "공격");

$rand_dm_list = $dm_list;
shuffle($rand_dm_list);
...
for($i=0; $i < count($rand_dm_list); $i++) {
	if($i >= $d_counter) break;

	$re_dm = $rand_dm_list[$i];
	$damage = rand($ds['dg_d_attack_min'], $ds['dg_d_attack_max']);
