...
// 일반 공격 처리 부분
$m_log_comment = "{$ds['dg_s2_attack_comment']}&&&&";
...
for($i=0; $i < count($dm_list); $i++) {
	...
	$target_field = "st_id_{$ds['dg_s2_attack_st_id']}";
	$target_field_mod = "st_id_{$ds['dg_s2_attack_st_id']}_mod";
	$target_field_use = "st_id_{$ds['dg_s2_attack_st_id']}_use";

	$is_etc_attack_able = false;
	$target_status = ($re_dm[$target_field] * 1) + ($re_dm[$target_field_mod]*1) - ($re_dm[$target_field_use]*1);
	$check_status = $ds['dg_s2_attack_st_value'] * 1;

	if($ds['dg_s2_attack_type'] == '이상' && $target_status >= $check_status) {
		$is_etc_attack_able = true;
	}
	if($ds['dg_s2_attack_type'] == '이하' && $target_status <= $check_status) {
		$is_etc_attack_able = true;
	}

	if($is_etc_attack_able) {
		$damage = rand($ds['dg_s2_attack_min'], $ds['dg_s2_attack_max']);
